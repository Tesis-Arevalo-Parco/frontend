...
const DashBoardHeader = () => {
	...
	const onSaveAssetsDependencies = async () => {
		activeSpinner(true)
		console.log('assetsNewDependencies', assetsNewDependencies)
	...
	const getHeader = () => {
		...
		} else if (location.pathname.includes(paths.STATE_POTENTIAL_RISK)) {
			return (
				<Col>
					<Row justify='space-between' align='middle'>
						<Row justify='space-between' align='middle'>
								...
								>
									{projects.map((project) => {
										return (
											<Option key={project.id} value={project.name}>
												<Link
													to={`${paths.STATE_POTENTIAL_RISK}/${project.id}`}
													style={{ display: 'block' }}
													onClick={() => setProjectName(project.name)}
												>
													{project.name}
		...
		} else if (location.pathname.includes(paths.STATE_RESIDUAL_RISK)) {
			return (
				<Col>
					<Row justify='space-between' align='middle'>
						<Row justify='space-between' align='middle'>
								...
								>
									{projects.map((project) => {
										return (
											<Option key={project.id} value={project.name}>
												<Link
													to={`${paths.STATE_RESIDUAL_RISK}/${project.id}`}
													style={{ display: 'block' }}
													onClick={() => setProjectName(project.name)}
												>
													{project.name}
